...
while($rowsInclude = mysql_fetch_array($queryInclude)){
  ...
  /*if($rowsInclude['var_type']==4){
	echo $rowsInclude['var_eval']."<br>";
  }
  */
}
//echo $_POST['bt_save_result']."<hr>";
...
if($_POST['bt_save_result'] != ""){
	...
	if($_POST['siteid'] != ''){
		$toFolder = "../../../repo_cmss/logic_result/".$_POST['siteid'];
		...
		if(is_dir($toFolder)){
			foreach($_FILES['ref_file']['name']  as $id => $name){
				$arr_temp = explode('.',$name);
				$arr_tempfile[$id] =  $_POST['attach_id'].$id.$_POST['service'].'.'.$arr_temp[count($arr_temp)-1];
			}
			
			foreach($_FILES['ref_file']['tmp_name']  as $id => $name){
				if($name != ''){
					if(file_exists($toFolder.'/'.$arr_tempfile[$id])){
						@unlink($toFolder.'/'.$arr_tempfile[$id]);
						@copy($name,$toFolder.'/'.$arr_tempfile[$id]);
						@chmod($toFolder.'/'.$arr_tempfile[$id],0777);
					}else{
						@copy($name,$toFolder.'/'.$arr_tempfile[$id]);
						@chmod($toFolder.'/'.$arr_tempfile[$id],0777);
					}
					...
					ref_file = '".$arr_tempfile[$id]."'
					WHERE  attach_id = '{$_POST[attach_id]}' AND cond_id = '{$id}' AND system_type = '{$_POST[service]}' ";
					mysql_db_query(STR_PREFIX_DB.$_POST['siteid'],$sql)or die(mysql_error());
				}
			}
		}
	}
...
<div class="art-BlockContent-body">
  ...
  <?php
	 ...
	 if($result){
   		$arrCond[] = "(1==1)";
        $txt = "<img src=\"images/accept.png\" align=\"absmiddle\" />&nbsp;<font color=\"#275980\"><b>ผ่าน/b></font>";
        echo "<script>document.getElementById('success').innerHTML = '<img src=\"images/accept.png\" align=\"absmiddle\" />';</script>";
		$resultCond = 1;
     }else{   
        $arrCond[] = "(1==0)";
        $txt = "<img src=\"images/error.png\" align=\"absmiddle\" />&nbsp;<font color=\"#FF0000\"><b>ไม่ผ่าน</b></font>";
...
<table width="980" class="logic">
	<tr>
        ...
        <th rowspan="2" width="40%">คำอธิบาย</th>
        <th colspan="2">การตรวจสอบ</th>
        <th rowspan="2">อ้างอิง</th>
    </tr>
    <tr>
	...
	<tr>
		...
		if($result){
			...
			if($rowchk_type[var_type]=="4"){
				$ref_text = "<b>หมายเหตุ</b><br><textarea id=\"ref_text".$i."\" name=\"ref_text".$i."\" style=\"width:200px; hieght:50px;\">".$array_result[$rowsCond['cond_id']]['ref_text']."</textarea>";
				$ref_text .= ($array_result[$rowsCond['cond_id']]['ref_file'] != '') ? '<br><br><b>ไฟล์แนบ</b> : <a href="'.$toFolder.$rows_command['secid']."/".$array_result[$rowsCond['cond_id']]['ref_file'].'" target="_blank">ดาวน์โหลดเอกสารแนบ</a>' : '<br><br><b>ไฟล์แนบ</b> :';
				$ref_text .="<input type=\"hidden\" id=\"hid_ref_file".$i."\" value=\"".$array_result[$rowsCond['cond_id']]['ref_file']."\">";
				$ref_text .= "<br><input type=\"file\" name=\"ref_file[".$rowsCond['cond_id']."]\" id=\"ref_file".$i."\">";
				$ref_text .="<br><b>ประเภทไฟล์</b> <font color=\"#ea0000\">.pdf, .xls, .xlsx, .doc, .docx</font>";
				$ref_text .="<br><b>ขนาดไฟล์</b> <font color=\"#ea0000\">10 MB</font>";
				...
				$txt.="<div style=\"display:block\">";
				$txt.="<input name=\"check_person".$i."\" class=\"pass_check star\" id=\"check_person1".$i."\" type=\"radio\" $check1 value=\"1\" onclick=\"return chk_per();\">&nbsp;ผ่าน&nbsp;&nbsp;";
				$txt.="<input name=\"check_person".$i."\" class=\"nopass_check star\" id=\"check_person2".$i."\" type=\"radio\" $check2 value=\"2\" onclick=\"return chk_per();\">&nbsp;ไม่ผ่าน";
			}else{
				$txt.="<div style=\"display:none\">";
				$txt.="<input name=\"check_person".$i."\" class=\"pass_check\" id=\"check_person1".$i."\" type=\"radio\" checked=\"checked\" value=\"1\" onclick=\"return chk_per();\">&nbsp;ผ่าน&nbsp;&nbsp;";
				$txt.="<input name=\"check_person".$i."\" class=\"nopass_check\" id=\"check_person2".$i."\" type=\"radio\" value=\"2\" onclick=\"return chk_per();\">&nbsp;ไม่ผ่าน";
			}
			$txt.='<input type="hidden" class="hid_cond_id" id="hid_cond_id'.$i.'" value="'.$rowsCond['cond_id'].'">';
			$txt.='<input type="hidden" id="system_result'.$i.'" value="1">';
			$txt.="</div>";
		 }else{
			...
			$txt.="<input name=\"check_person".$i."\" class=\"nopass_check star\" id=\"check_person2".$i."\" type=\"radio\" $check2 value=\"2\" onclick=\"return chk_per();\">&nbsp;ไม่ผ่าน ";
			$txt.='<input type="hidden" class="hid_cond_id"  id="hid_cond_id'.$i.'" value="'.$rowsCond['cond_id'].'">';
			$txt.='<input type="hidden" id="system_result'.$i.'" value="0">';
			$txt.="</div>";
...
<?php
	if($_GET[idcard] && $_GET[attach_id]){
		$action="?letter_id=".$_GET['letter_id']."&idcard=".$idcard."&attach_id=".$_GET['attach_id']."&service=".$_GET['service']."&meeting_id=".$_GET['meeting_id']."";
	}else{	
		$action="?letter_id=".$_GET['letter_id']."&idcard=".$idcard."&by_person=".$_GET['by_person']."&pos=".($_GET['pos']-1)."&now=".$_GET['now']."&par=".$_GET['par']."&service=".$_GET['service']."&meeting_id=".$_GET['meeting_id']."";
	}
?>
<!-- ?letter_id=<?=$_GET['letter_id']?>&idcard=<?=$idcard?>&by_person=<?=$_GET['by_person']?>&pos=<?=($_GET['pos']-1)?>&now=<?=$_GET['now']?>&par=<?=$_GET['par']?>&service=<?=$_GET['service']?>&meeting_id=<?=$_GET['meeting_id']?>
-->
<table width="100%" border="0" cellspacing="0" cellpadding="3">
  ...
  <tr>
	...
	<input name="attach_id" id="attach_id" type="hidden" value="<?=$attach_id?>" />
    <input name="service" id="service" type="hidden" value="<?=$_GET['service']?>" />
    <input name="siteid" id="siteid" type="hidden" value="<?=$rows_command['secid']?>" />
  ...
  <tr>
	...
	?>
	 <td><input name="check_by_person" id="check_by_person1" type="radio" value="1" <?php echo ($verify_data[status_per]=='1')?'checked':'';?>/>&nbsp;ผ่าน<?php echo $if_condition ?>&nbsp;&nbsp;<input name="check_by_person" id="check_by_person2" type="radio" value="2" <?php echo ($verify_data[status_per]=='2')?'checked':'';?>/>&nbsp;ไม่ผ่าน</td>
	
    <!--<td><input name="check_by_person" type="radio" value="1" <?php echo ($verify_data[status_per]=='1')?'checked':'';?>/>&nbsp;ผ่าน<?php echo $if_condition ?>&nbsp;&nbsp;<input name="check_by_person" type="radio" value="2" <?php echo ($verify_data[status_per]=='2')?'checked':'';?>/>&nbsp;ไม่ผ่าน</td>
	-->
	<? //@end ?>
  </tr>
  <tr>
    <td><div align="right"><strong>ชื่อเจ้าหน้าที่ผู้ตรวจสอบ</strong></div></td>
    <td><input name="name" type="text" value="<?=$_SESSION['office_name']?>" size="70" /></td>
  </tr>
  <tr>
    <td valign="top"><div align="right"><strong>หมายเหตุ</strong></div></td>
    <td><textarea name="comment" cols="75" rows="5"><?php echo $verify_data[comment];?></textarea></td>
  </tr>
  <tr>
    <td valign="top">&nbsp;</td>
    <td><input name="bt_save_result" id="bt_save_result" type="submit" value="  บันทึกผลการตรวจสอบ  " /></td>
  </tr>
</table>
<?php //@end?>
</form>
...
if($_GET['pos'] < $all_pos && $_GET['pos']!=0){
?>
...
<script>
	function chk_per(){
		var numberNotChecked = $('.nopass_check:radio:not(":checked")').length;
		var numnopass_check = $('.nopass_check:radio').length;
		
		if(numberNotChecked == numnopass_check){
			$('#check_by_person1').attr('checked','checked');
			$('#check_by_person2').attr('disabled','disabled');
			$('#check_by_person1').removeAttr('disabled');
		}else{
			$('#check_by_person2').attr('checked','checked');
			$('#check_by_person1').attr('disabled','disabled');
			$('#check_by_person2').removeAttr('disabled');
		}
		
		if($('.star:radio:checked').length == ($('.star:radio').length/2)){
			$('#bt_save_result').removeAttr('disabled');
		}else{
			$('#bt_save_result').attr('disabled','disabled');
		}
	}
	
	function save_logic_result(){
		var attach_id = $('#attach_id').val();
		var obj = Array;
		var error = '';
		var error_obj = 0;
		
		obj['service'] = '<?=$_GET['service']?>';
		$('.hid_cond_id').each(function(){
			var hid_id = $(this).attr('id');
			var num_id = hid_id.replace('hid_cond_id','');
			
			if($("#ref_text"+num_id).length) {
				if($.trim($('#ref_text'+num_id).val()) == '' && error == ''){
					error = 'ref_text';
					error_obj = num_id;
				}
			}
			
			if($("#ref_file"+num_id).length) {
				if($.trim($('#ref_file'+num_id).val()) == '' && error == '' && $.trim($('#hid_ref_file'+num_id).val()) == ''){
					error = 'ref_file';
					error_obj = num_id;
				}
			}
			
			if($('#check_person1'+num_id).attr('checked') == true){
				var person_result = 1;
			}else{
				var person_result = 0;
			}
			
			var ref_text = ($("#ref_text"+num_id).length) ? $('#ref_text'+num_id).val() : '';
			
			obj[num_id] = {
				'attach_id' : attach_id,
				'cond_id' : $('#'+hid_id).val(),
				'system_type' : '<?=$_GET['service']?>',
				'system_result' : $('#system_result'+num_id).val(),
				'person_result' : person_result,
				'ref_text' : ref_text,
				'siteid' : '<?=$rows_command['secid']?>'
			};
		});
		
		if(error == 'ref_text'){
			alert('กรุณากรอกหมายเหตุอ้างอิง ของตรรกะ '+$('#logicTD'+error_obj).text());
			$('#ref_text'+error_obj).focus();
			return false;	
		}else if(error == 'ref_file'){
			alert('กรุณาแนบไฟล์อ้างอิง ของตรรกะ '+$('#logicTD'+error_obj).text());
			$('#ref_text'+error_obj).focus();
			return false;	
		}
		
		$.ajax({
			url: "ajax_save_logic.php",
			data: obj,
			type: 'post',
	...
	$(document).ready(function(){
		if($('.star:radio').length > 0 && ($('.star:radio:checked').length < ($('.star:radio').length/2))){
			$('#bt_save_result').attr('disabled','disabled');
		}else{
			$('#bt_save_result').removeAttr('disabled');
		}
	...
	$('input[type=file]').change(function(){
		...
		arr_type = name.split('.');
		file_type = arr_type[arr_type.length-1];
		
		if(file_type == 'pdf' || file_type == 'xls' || file_type == 'xlsx' || file_type == 'doc' || file_type == 'docx'){
			
		}else if((size/1048576) > 10){
			alert('ขนาดไฟล์ที่แนบ มีขนาดใหญ่เกินกว่าที่ระบบกำหนด');
			
		}else{
			alert('ประเภทไฟล์ที่แนบ ไม่ตรงตามที่ระบบกำหนด');
			$(this).val('');
		}
	});
</script>
