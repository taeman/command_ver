		...
		if($arrSalary[runno]!=""){
				...
				if($res_select_sch['schoolid']!="" && $res_select_sch['schoolid']!="0"){
						...
						if($arrSalary2[strdate]!=""){
								...
								$this->arrDataExp[SHOW_START_DATE] = $this->showStrDateForStartDate($this->getPeriodReal($arrSalary2[strdate],$this->startDateTh));
						}else{
 ...
 * @return string
 */
private function showStrDateForStartDate($arrDate){
		$str="";
		if(is_array($arrDate)){
				$str=$arrDate[2]." ปี  ".$arrDate[1]." เดือน  ".$arrDate[0]." วัน";
		}
		
		return $str;
}	// end function showStrDateTH
